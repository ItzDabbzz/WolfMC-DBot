    ...
    public static String parseVariables(@Nonnull String message,
        ...
        if (matcher.find()) {
            ...
            if (matcher.groupCount() >= 2) {
                min = (matcher.group(2) != null ? Utils.getInt(matcher.group(2), min) : min);
                max = (matcher.group(3) != null ? Utils.getInt(matcher.group(3), max) : max);
            }
            parsed = matcher.replaceAll(String.valueOf(Utils.getInt(min, max)));
        }
        ...
        while (matcher.find()) {
            int argIndex = Utils.getPositiveInt(matcher.group(1), 0);
